...
ATON.utils.detectDeviceCapab = function(){
    ...
    ATON._maxTexSize = 4096;

    ATON._glCTX = ATON._canvas.getContext('experimental-webgl');
    if (ATON._glCTX == null){ 
        ATON._glCTX = ATON._canvas.getContext('webgl', { xrCompatible: true });
        }

    if(ATON._glCTX != null){ // WebGL is supported 

        let highp = ATON._glCTX.getShaderPrecisionFormat(ATON._glCTX.FRAGMENT_SHADER, ATON._glCTX.HIGH_FLOAT);
        ATON._bHighP     = (highp.precision != 0);
        
        let maxTexSize = ATON._glCTX.getParameter(ATON._glCTX.MAX_TEXTURE_SIZE);
        if (maxTexSize) ATON._maxTexSize = maxTexSize;

        if (ATON._maxTexSize <= 4096) ATON._isMobile = true;

        // WebXR
        ATON._bXRsupported = false;
        ATON._xrSession = undefined;

        if (navigator.xr){
            navigator.xr.isSessionSupported('immersive-vr').then((bXR)=>{
                if (bXR){
                    ATON._bXRsupported = true;
...
/*
ATON.toggleXR = function(){

    if (!ATON._xrSession){
        navigator.xr.requestSession('immersive-vr').then((session)=>{
            ATON._xrSession = session;

            let xrLayer = new XRWebGLLayer(session, ATON._glCTX);
